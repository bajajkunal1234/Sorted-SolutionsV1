 ...
 * Assumes address format: "Building, Area, Locality, City, State Pincode"
 */
...
export const groupJobsBy = (jobs, groupBy) => {
    ...
    jobs.forEach(job => {
        ...
        switch (groupBy) {
            case 'assignee':
                key = job.assignedToName || 'Unassigned';
                break;
            case 'status':
...
export const filterJobs = (jobs, { searchTerm, status, dateRange, assignee }) => {
    ...
    if (searchTerm) {
        ...
        filtered = filtered.filter(job =>
            ...
            job.property?.address?.toLowerCase().includes(term) ||
            job.assignedToName?.toLowerCase().includes(term)
        );
    }
    ...
    if (assignee && assignee !== 'all') {
        filtered = filtered.filter(job => job.assignedTo === assignee);
    }
