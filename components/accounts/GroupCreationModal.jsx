...
import { accountGroups as sampleGroups, primaryCOAGroups } from '../../data/accountingData';
import { checkDuplicateName, getGroupPath } from '../../utils/accountHelpers';

function GroupCreationModal({ onClose, onSave, existingGroups = sampleGroups }) {
    ...
    const handleNameChange = (name) => {
        ...
        if (name.trim()) {
            const duplicate = checkDuplicateName(name, existingGroups);
    ...
    const handleSubmit = async (e) => {
        ...
        // Create new group
        // Create new group payload
        const newGroup = {
            id: formData.name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, ''),
            name: formData.name,
            alias: formData.alias || null,
            parent: formData.under,
            nature: existingGroups.find(g => g.id === formData.under)?.nature || 'asset',
        ...
        try {
            ...
            const response = await fetch('/api/admin/account-groups', {
                method: 'POST',
                        ...
                        {formData.name && formData.under && (
                            <div style={{
                                ...
                                <div style={{ fontSize: 'var(--font-size-sm)', display: 'grid', gap: '4px' }}>
                                    <div>
                                        <strong>Full Path:</strong> {getGroupPath(formData.under, accountGroups)} &gt; {formData.name}
                                    </div>
                                    <div>
                                        <strong>Nature:</strong> {accountGroups.find(g => g.id === formData.under)?.nature || 'N/A'}
                                    </div>
                                    <div>
