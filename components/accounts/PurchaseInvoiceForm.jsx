    ...
    const [invoiceMode, setInvoiceMode] = useState(() => {
        if (existingInvoice) {
            const hasExpenseItem = existingInvoice.items?.some(item => {
                const ledger = sampleLedgers.find(l => l.id === item.productId);
                return ledger?.type === 'expense';
            });
            if (hasExpenseItem) return 'ledger';
        }
        return 'item';
    });
        ...
        <div style={{
            ...
            <div style={{
                ...
                <div style={{ flex: 1, overflow: 'auto', padding: 'var(--spacing-lg)' }}>
                                                    ...
                                                    <td style={{ padding: 'var(--spacing-xs)', position: 'relative', overflow: 'visible' }}>
                                                        <div style={{ marginBottom: 'var(--spacing-xs)' }}>
                                                            <AccountSelector
                                                                ...
                                                                onChange={(accountId) => {
                                                                    const acc = sampleLedgers.find(l => l.id === accountId);
                                                                    if (!acc) return;
                                                                    const isGstApplicable = acc.gstApplicable !== false && acc.gst_applicable !== false;
                                                                    const defaultTaxRate = isGstApplicable ? (acc.taxRate !== undefined ? parseFloat(acc.taxRate) : (acc.tax_rate !== undefined ? parseFloat(acc.tax_rate) : 18)) : 0;
                                                                    const newItems = [...formData.items];
                                                                    newItems[index] = {
                                                                        ...newItems[index],
                                                                        productId: acc.id,
                    ...
                    <div>
                        <label style={{ display: 'block', fontSize: 'var(--font-size-sm)', fontWeight: 500, marginBottom: 'var(--spacing-xs)' }}>
                            {invoiceMode === 'ledger' ? 'Narration *' : 'Notes (Optional)'}
                        </label>
                        <textarea
                            className="form-input"
