...
// ─── Inventory filter field definitions ═══════════════════════════════════════
...
const INVENTORY_PRESETS = [
    { id: 'in_stock',      label: '📦 In Stock',         filter: { stock_status: 'In Stock' } },
    { id: 'low_stock',     label: '⚠️ Low Stock',        filter: { stock_status: 'Low Stock' } },
    { id: 'out_of_stock',  label: '❌ Out of Stock',     filter: { stock_status: 'Out of Stock' } },
    { id: 'products_only', label: '🛒 Products Only',    filter: { type: 'product' } },
    { id: 'services_only', label: '🔧 Services Only',    filter: { type: 'service' } },
];

const INVENTORY_GROUP_BY = [
...
function CustomFilterRow({ row, onChange, onRemove }) {
    const field = INVENTORY_FILTER_FIELDS.find(f => f.key === row.field) || INVENTORY_FILTER_FIELDS[0];
    ...
    return (
        <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
            <select value={row.field} onChange={e => onChange({ ...row, field: e.target.value, operator: 'is', value: '' })} style={{ ...sel, flex: '1 1 120px' }}>
                {INVENTORY_FILTER_FIELDS.map(f => <option key={f.key} value={f.key}>{f.label}</option>)}
            </select>
...
export default function InventorySearchPanel({
    ...
    const [customRows, setCustomRows] = useState([{ id: Date.now(), field: 'name', operator: 'contains', value: '' }]);
    ...
    const addCustomRow = () => setCustomRows(prev => [...prev, { id: Date.now(), field: 'name', operator: 'contains', value: '' }]);
    ...
    const applyCustomFilters = () => {
        ...
        const label = valid.map(r => {
            const fl = INVENTORY_FILTER_FIELDS.find(f => f.key === r.field)?.label || r.field;
        ...
        setCustomRows([{ id: Date.now(), field: 'name', operator: 'contains', value: '' }]);
    ...
    const activeGroupByLabel = INVENTORY_GROUP_BY.find(o => o.value === groupBy)?.label;
    const activeSortByLabel = INVENTORY_SORT_BY.find(o => o.value === sortBy)?.label;
                    ...
                    <div style={{ padding: '10px 16px' }}>
                        {!showSaveInput ? (
                            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                                ...
                                <button onClick={() => { onResetView && onResetView(); setOpen(false); }}
