...
function AccountSelector({ value, onChange, onCreateNew, accountType = 'all', label = 'Account' }) {
    // Real accounts from sampleLedgers
    const allAccounts = sampleLedgers.map(ledger => ({
        id: ledger.id,
        name: ledger.name,
        type: ledger.type,
        gstin: ledger.gstin || ledger.gstNumber || '',
        properties: ledger.properties || [],
        address: ledger.address,
        state: ledger.address?.state || 'Maharashtra'
    }));

    // Filter accounts based on type
    const accounts = accountType === 'all'
        ? allAccounts
        : allAccounts.filter(acc => {
            if (accountType === 'customer') {
                const ledger = sampleLedgers.find(l => l.id === acc.id);
                return acc.type === 'customer' ||
