...
function StepPhoto({ name, customerId, onNext, onSkip }) {
    ...
    const handleFile = async (e) => {
        ...
        setUploading(true)
        try {
            // Client-side compression to avoid Vercel 4.5MB payload limit
            ...
            if (!up.ok) {
                ...
                throw new Error('Upload server error: ' + text);
            }
            
            const upData = await up.json()
            if (!upData.url) throw new Error('Upload failed: missing url')
                ...
                <button 
                    onClick={() => {
                        if (step > 1) {
                            ...
                            // Back on step 1 means cancel/logout
                    ...
                    style={{ position: 'absolute', top: 24, left: 20, background: 'none', border: 'none', color: '#94a3b8', display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, cursor: 'pointer', zIndex: 1100 }}
                >
