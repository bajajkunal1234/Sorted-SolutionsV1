    ...
    jobs.forEach(job => {
        ...
        switch (groupBy) {
            case 'assignee':
                ...
                break;
            case 'status':
                key = job.status ? job.status.replace(/_/g, ' ').replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase()) : 'Unknown';
                break;
            case 'dueDate':
        ...
        switch (sortBy) {
            ...
            case 'scheduledTime': {
                const parseTime = (j) => {
                    ...
                    return (bd.schedule?.slot || j.scheduled_time || '').toLowerCase().trim();
                };
            ...
            case 'invoice':
                ...
                break;
            case 'appliance':
                aVal = (a.appliance || '').toLowerCase();
                bVal = (b.appliance || '').toLowerCase();
                break;
            case 'applianceType':
...
export const getInitials = (name) => {
    ...
    if (cleanName.includes('v.d') || cleanName.includes('vd') || cleanName.includes('sakinaka') || cleanName.includes('v d gupta')) {
        return 'VD';
    }
    return name
        .split(' ')
...
/**
 * Canonical kanban column order for 9-status lifecycle.
 * Import this wherever columns need to be sorted in the correct flow sequence.
 */
export const STATUS_ORDER = [
    ...
    'cancelled',
    'closed',
    // legacy — map to nearest canonical slot for display
    'booking_request',
    'assigned',
    'in-progress',
    'quotation-sent',
...
/**
 * Get status color — updated for 9-status lifecycle.
 */
export const getStatusColor = (status) => {
    const colors = {
        // ── Canonical 9-status values ──
        'enquiry':            '#94a3b8',
        'new_job_request':    '#f59e0b',
        'scheduled':          '#38bdf8',
        'diagnosing_quoting': '#8b5cf6',
        'quotation_sent':     '#a78bfa',
        'parts_ordered':      '#f97316',
        'work_in_progress':   '#10b981',
        'cx_reschedule':      '#06b6d4',
        'cancelled':          '#ef4444',
        'closed':             '#10b981',
        // ── Legacy fallbacks ──
        'pending':            '#f59e0b',
...
/**
 * Get background gradient color for technician badge
 */
export const getTechnicianColor = (name) => {
    ...
    if (cleanName.includes('v.d') || cleanName.includes('vd') || cleanName.includes('sakinaka')) {
        return 'linear-gradient(135deg, #f59e0b, #b45309)'; // Golden yellow/amber gradient
    }
