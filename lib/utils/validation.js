...
export const formatMobileNumber = (mobile) => {
    ...
    // 1. Remove the prefix +91- or +91 if it exists at the start
    let cleaned = mobile.trim();
    if (cleaned.startsWith('+91')) {
        cleaned = cleaned.slice(3);
    }
    // Remove leading dash/spaces if present after +91
    cleaned = cleaned.replace(/^[\s-]+/, '');

    // 2. Strip all non-digits from the remaining string
    let digits = cleaned.replace(/\D/g, '');

    // 3. Handle leading 0 or leading 91 (if they typed/pasted a 12-digit number starting with 91)
