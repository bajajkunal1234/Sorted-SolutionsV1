...
export const generateSKU = (accountType, existingAccounts = [], settings = {}) => {
    ...
    const existingSKUs = (existingAccounts || [])
        .filter(acc => acc && acc.sku && acc.sku.startsWith(prefix))
        .map(acc => {
...
export const generateShortKU = (underGroup, existingAccounts = [], groups = []) => {
    ...
    const safeAccounts = existingAccounts || [];
    const existingNums = safeAccounts
        .filter(acc => acc && acc.sku && typeof acc.sku === 'string' && acc.sku.startsWith(prefix))
        .map(acc => {
...
export const getRequiredFields = (underGroup, groups = []) => {
    const fieldMappings = {
        'sundry-debtors': [
            ...
            'customerDescription',
            'acquisitionSource', 'referredBy', 'status'
        ],
        'customer-accounts': [
            'accountImage', 'contactPerson', 'mobile', 'email', 'mailingName',
            'gstRegistration', 'gstin', 'pan', 'stateName', 'country',
        ...
        'sundry-creditors': [
            ...
            'gstRegistration', 'gstin', 'pan', 'stateName', 'country',
            'customerDescription',
            'creditPeriod', 'properties', 'acquisitionSource', 'referredBy', 'status'
        ],
        'supplier-accounts': [
            'accountImage', 'contactPerson', 'mobile', 'email', 'mailingName',
            'gstRegistration', 'gstin', 'pan', 'stateName', 'country',
        ...
        'technicians': [
            'accountImage', 'contactPerson', 'mobile', 'email', 'status'
        ],
        'technician-accounts': [
            'accountImage', 'contactPerson', 'mobile', 'email', 'status'
        ],
...
export const validateAccountData = (data, underGroup, groups = []) => {
    ...
    if (requiredFields.includes('mobile') && data.mobile) {
        // Strip non-digits, then remove leading country code (91 or 0)
        let digits = data.mobile.replace(/\D/g, '');
