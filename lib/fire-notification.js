...
const FALLBACK_COPY = {
    ...
    job_scheduled:            { title: 'Appointment Scheduled ✅',       body: 'Your appointment is confirmed. Our technician will visit you soon.' },
    job_diagnosing:           { title: 'Technician Arrived',             body: 'Our technician has arrived and is diagnosing the issue.' },
    quotation_sent:           { title: 'Quotation Ready 📋',             body: 'Your repair estimate is ready. Please review and approve it in the app.' },
    quotation_approved:       { title: 'Quotation Approved ✅',          body: 'The quotation has been approved. Repair work will begin shortly.' },
    parts_ordered:            { title: 'Parts Ordered 🔩',               body: 'Parts have been ordered for your repair. We\'ll update you when they arrive.' },
    work_in_progress:         { title: 'Repair In Progress 🔧',          body: 'Our technician is actively working on your appliance.' },
    job_rescheduled_cx:       { title: 'Appointment Rescheduled',        body: 'A customer has rescheduled their appointment. Please review the new slot.' },
    job_closed:               { title: 'Service Completed ✅',           body: 'Your service job is complete. Thank you for choosing Sorted!' },
    job_cancelled:            { title: 'Job Cancelled',                  body: 'Your service request has been cancelled. Contact us if this was a mistake.' },
    repair_note_added:        { title: 'Repair Note Added',              body: 'The technician has logged a repair note for this job.' },
...
export async function fireNotification(event_type, context = {}) {
    ...
    try {
            ...
            for (const trigger of triggers) {
                ...
                if (audience.includes('customers') && customer_id) {
                    const { data: customer } = await supabase
                        ...
                        .maybeSingle();
                    recipientSets.push({
                        ...(customer || { id: customer_id, name: customer_name || 'Customer' }),
                ...
                await Promise.all(recipientSets.map(async (recipient) => {
                    ...
                    // Always save to in-app bell (even if push/whatsapp failed or was skipped — no FCM token is fine)
                    const appNotifPromise = supabase.from('app_notifications').insert({
                    ...
                    const logPromise = supabase.from('notification_logs').insert({
                    ...
                    const [insertRes, logRes] = await Promise.all([appNotifPromise, logPromise]);
                    if (insertRes.error) console.error('[fireNotification] Failed to save in-app notification:', insertRes.error.message);
                    if (logRes.error) console.error('[fireNotification] Log insert error:', logRes.error.message);
                }));
            }
        } else {
        ...
        const TECH_EVENTS = [
            'job_assigned', 'job_scheduled', 'job_diagnosing',
            'quotation_approved', 'parts_ordered', 'work_in_progress',
            'job_rescheduled_cx', 'job_closed', 'job_cancelled',
            // legacy
        ...
        if (fallbackPromises.length > 0) {
            await Promise.all(fallbackPromises).catch(e => console.error('[fireNotification] Fallback insert failed:', e.message));
        }
