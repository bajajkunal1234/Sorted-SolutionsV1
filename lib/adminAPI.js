...
import { apiCall } from './offlineSync';

const API_BASE = process.env.NEXT_PUBLIC_API_URL || '/api/admin'

// Generic fetch wrapper with automatic retries for GET requests
async function apiFetch(endpoint, options = {}) {
    ...
    let attempts = 0
    const maxAttempts = isGet ? 2 : 1 // Retry GET requests once
    ...
    while (attempts < maxAttempts) {
        ...
        try {
            ...
            if (data.duplicated) {
                throw new Error('A request with these details is already being processed. Please wait a moment.');
            }

            return data.data !== undefined ? data.data : data
        } catch (error) {
            // Only retry GET requests if we have attempts left
            if (isGet && attempts < maxAttempts) {
                console.warn(`[apiFetch] GET request to ${endpoint} failed (attempt ${attempts}/${maxAttempts}). Retrying in 1.5s... Error:`, error.message);
...
export const jobsAPI = {
    ...
    getAll: async (filters = {}) => {
        ...
        if (filters._t) params.append('t', filters._t)
...
export const accountsAPI = {
    getAll: async (type = '', include_archived = false, search = '') => {
        const params = new URLSearchParams()
        if (type) params.append('type', type)
        if (include_archived) params.append('include_archived', '1')
...
export const journalsAPI = {
    ...
    delete: async (id) => {
        return apiFetch(`/journals?id=${id}`, {
            method: 'DELETE',
        })
    }
}
...
export const printSettingsAPI = {
    get: async () => {
        return apiFetch('/print-settings')
    },
