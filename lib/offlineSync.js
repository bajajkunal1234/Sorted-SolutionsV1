...
/**
 * Helper to upload a file immediately if online, or queue it in IndexedDB if offline/slow.
 */
export async function uploadOrQueueFile(file, customFileName) {
    ...
    // If online, try immediate upload (with 6s timeout)
    if (isOnline()) {
        const formData = new FormData();
        const safeFileName = customFileName || (file.name ? file.name.replace(/[^a-zA-Z0-9.\-_]/g, '') : 'upload.jpg');
        ...
        try {
            ...
            clearTimeout(timeoutId);
            console.warn('[Offline] Immediate file upload failed, queuing locally:', e);
        }
    }
