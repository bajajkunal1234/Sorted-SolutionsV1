...
function queueRequest(url, options) {
    try {
        ...
        const isDuplicate = queue.some(item => item.url === url && getNormalizedBody(item.body) === normBody);
        if (isDuplicate) {
            console.warn('[Offline] Identical operation already pending in offline sync queue, skipping duplicate push:', url);
...
export async function uploadOrQueueFile(file, customFileName) {
    ...
    try {
        await storeOfflineFile(fileId, file, fileName);
    } catch (err) {
