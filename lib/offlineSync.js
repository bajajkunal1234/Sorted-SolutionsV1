...
export async function apiCall(url, options = {}) {
    ...
    if (isOnline() && !hasPlaceholder) {
        const controller = new AbortController();
        const timeoutMs = isCreation ? 45000 : 15000;
        ...
        try {
            ...
            // Critical entity creation requests (like new jobs or invoices) must NOT be blindly queued
            // upon a network timeout, because the server may have already received and committed it.
            if (isCreation) {
                console.warn('[Offline] Network error/timeout for creation request. Raising error to caller instead of queuing duplicate.');
                const entityName = url.includes('/jobs') ? 'jobs list' : (url.includes('/transactions') ? 'invoices / transactions list' : 'system');
                throw new Error(`Network timeout or connection error while communicating with server. Please verify the ${entityName} before retrying.`);
            }
