    ...
    for (const item of queue) {
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (res.status >= 500) {
                    // Server-side transient error (e.g., 502/503/504), keep in queue to retry later
                    hasNetworkError = true;
                    remainingQueue.push(item);
                    continue;
                }
...
/**
 * Helper to upload a file directly if online, or queue in IndexedDB if offline.
 * Returns public URL (if online) or placeholder URL (if offline).
 */
export async function uploadOrQueueFile(file, customFileName) {
    ...
    const fileName = customFileName || file?.name || 'upload.jpg';

    // If online and valid file, attempt direct upload immediately
    if (isOnline() && file && (file instanceof Blob || (typeof File !== 'undefined' && file instanceof File)) && file.size > 0) {
        try {
            const formData = new FormData();
            ...
            console.warn('[Offline] Direct upload returned non-ok, falling back to offline queue');
        } catch (err) {
            console.warn('[Offline] Direct upload failed, falling back to offline queue:', err);
        }
    }
