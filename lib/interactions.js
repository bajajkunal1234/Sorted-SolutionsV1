...
export const logInteraction = async ({
    ...
    try {
        let localTechId = null;
        let sessionToken = null;
        try {
            const session = typeof window !== 'undefined' && localStorage.getItem('technicianSession');
            if (session) {
                const data = JSON.parse(session);
                localTechId = data.technicianId;
                sessionToken = data.session_token;
            }
        } catch (e) {}

        if (!performedBy && localTechId) {
            performedBy = localTechId;
        }
        ...
        let lastCoords = null;
        try {
            const coordsStr = typeof window !== 'undefined' && localStorage.getItem('lastKnownCoordinates');
            if (coordsStr) {
                lastCoords = JSON.parse(coordsStr);
            }
        } catch (e) {}

        if (lastCoords && lastCoords.latitude && lastCoords.longitude && !metadata.latitude && !metadata.longitude) {
            metadata = {
                ...metadata,
        ...
        if (metadata && metadata.latitude && metadata.longitude && !metadata.locality) {
            try {
                ...
                console.error('[logInteraction] Geocoding error:', geoErr);
            }
        }
