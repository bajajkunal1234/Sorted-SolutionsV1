...
/** Given a pincode, returns the first matching locality name or '' if not found */
export function getLocalityForPincode(pincode) {
    const match = MUMBAI_LOCALITIES.find(l => l.pincode === pincode);
    return match ? match.name : '';
}

import { MUMBAI_LOCALITY_COORDS } from './mumbaiLocalityCoords';
export { MUMBAI_LOCALITY_COORDS };

/**
 * Resolves coordinates for any Mumbai locality name, pincode, or address query.
 * Checks exact name match, substring match, and 6-digit pincode lookup.
 */
export function getCoordinatesForLocality(query) {
    if (!query) return null;
    const clean = String(query).trim().toLowerCase();

    // 1. Exact match on locality name
    for (const [name, coords] of Object.entries(MUMBAI_LOCALITY_COORDS)) {
        if (name.toLowerCase() === clean) return coords;
    }

    // 2. Partial match on locality name (e.g. "Kharghar" matching "Navi Mumbai (Kharghar)")
    for (const [name, coords] of Object.entries(MUMBAI_LOCALITY_COORDS)) {
        const nameLower = name.toLowerCase();
        if (nameLower.includes(clean) || clean.includes(nameLower)) {
            return coords;
        }
    }

    // 3. Check 6-digit pincode in query
    const pinMatch = clean.match(/\b\d{6}\b/);
    if (pinMatch) {
        const pin = pinMatch[0];
        const matchedLoc = MUMBAI_LOCALITIES.find(l => l.pincode === pin);
        if (matchedLoc && MUMBAI_LOCALITY_COORDS[matchedLoc.name]) {
            return MUMBAI_LOCALITY_COORDS[matchedLoc.name];
        }
    }
