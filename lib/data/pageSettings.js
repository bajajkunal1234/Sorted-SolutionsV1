 ...
 * IMPORTANT: throws when page data is null so unstable_cache does NOT store
 * a 404/missing-row state. Only successful (data !== null) results are cached.
 * This prevents a transient DB miss from locking a page into a phantom 404 for 5 minutes.
 */
export const cachedGetFullPageData = unstable_cache(
    async (pageId) => {
        const result = await getFullPageData(pageId)
        if (!result.success || !result.data) {
            // Throw — unstable_cache won't store rejected/thrown results,
            // so the next request will retry Supabase rather than hitting a cached null.
            throw new Error(`[cachedGetFullPageData] Page not found: ${pageId}`)
        }
        return result
    },
    ['page-full-data'],
