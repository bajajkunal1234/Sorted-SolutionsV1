...
/**
 * Fetches all associated profiles for a device
 */
export async function getDeviceProfiles(deviceId) {
    if (!deviceId) return null;
    const token = await getAccessToken();
    const apiHost = process.env.MANAGEENGINE_API_HOST || 'https://mdm.manageengine.in';
    
    if (!token) {
        // Return simulated profile status if not authenticated (simulator)
        return {
            profiles: [
                {
                    profile_name: "System_Policy_v1",
                    profile_id: process.env.MANAGEENGINE_ON_DUTY_PROFILE_ID || "51167000000097017",
                    status: "6",
                    localized_remarks: "Successfully applied the policy (SIMULATED)"
                }
            ]
        };
    }
    ...
    try {
        ...
        console.error('[MDM] getDeviceProfiles error:', err.message);
        return null;
    }
}
