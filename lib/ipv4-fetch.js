...
export function createIPv4Fetch() {
    ...
    return async function ipv4Fetch(inputUrl, init = {}) {
        ...
        return new Promise((resolve, reject) => {
            ...
            req.on('error', (err) => {
                // Network error — fall back to default fetch
                console.warn('[ipv4-fetch] https.request failed, falling back:', err.message);
                fetch(inputUrl, init).then(resolve).catch(reject);
            });
