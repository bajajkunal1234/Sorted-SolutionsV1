...
export function createIPv4Fetch() {
    ...
    return async function ipv4Fetch(inputUrl, init = {}) {
        ...
        // Vercel production does not suffer from ISP DNS interception.
        // Skip custom resolver to avoid timeouts if Vercel blocks UDP port 53.
        ...
        if (body) {
            try {
                if (typeof body === 'string' || Buffer.isBuffer(body)) {
                    ...
                    // Node stream - will be piped during request sending
                } else {
                    console.warn('[ipv4-fetch] Unhandled body type:', typeof body, body.constructor?.name);
                }
            } catch (e) {
                console.error('[ipv4-fetch] Failed to pre-process request body:', e);
            }
        }
        ...
        return new Promise((resolve, reject) => {
            console.log(`[ipv4-fetch] Sending ${method} to ${url.pathname}`);
            const req = https.request(requestOptions, (res) => {
                console.log(`[ipv4-fetch] Received status ${res.statusCode} for ${method}`);
                const chunks = [];
                res.on('data', (chunk) => {
                    console.log(`[ipv4-fetch] Received chunk of length ${chunk.length}`);
                    chunks.push(chunk);
                });
                res.on('end', () => {
                    try {
                        ...
                        // Node 'Response' strictness: body must be null for 204, 205, or 304
