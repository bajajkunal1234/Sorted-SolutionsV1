...
// Prevent Next.js App Router from aggressively caching Supabase GET requests
const noCacheFetch = (url, options) => {
    return customFetch(url, { ...options, cache: 'no-store' });
};

// On the server, use SUPABASE_SERVICE_ROLE_KEY if available to ensure server API routes have trusted access
const effectiveKey = (typeof window === 'undefined' && process.env.SUPABASE_SERVICE_ROLE_KEY)
    ? process.env.SUPABASE_SERVICE_ROLE_KEY
    : supabaseAnonKey;

// Prevent build-time crashes if keys are missing
export const supabase = (supabaseUrl && effectiveKey)
