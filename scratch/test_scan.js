...
async function run() {
    ...
    console.log(`2. Triggering OTA scan for device "${deviceId}"...`);
    const scanUrl = `${apiHost}/api/v1/mdm/devices/${deviceId}/actions/scan`;
    ...
    try {
        ...
        console.log('Scan Action HTTP Status Code:', response.status);
        const text = await response.text();
        console.log('Scan Action Response:', text);
    } catch (err) {
