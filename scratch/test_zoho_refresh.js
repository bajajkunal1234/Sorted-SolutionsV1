require('dotenv').config({ path: '.env.local' });

async function testRefresh() {
    const clientId = process.env.MANAGEENGINE_CLIENT_ID;
    const clientSecret = process.env.MANAGEENGINE_CLIENT_SECRET;
    const refreshToken = process.env.MANAGEENGINE_REFRESH_TOKEN;
    const authUrl = process.env.MANAGEENGINE_AUTH_URL || 'https://accounts.zoho.com/oauth/v2/token';

    console.log('Testing Zoho Refresh Token Exchange using configured environment variables...');

    try {
        const response = await fetch(authUrl, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
            },
            body: new URLSearchParams({
                refresh_token: refreshToken,
                client_id: clientId,
                client_secret: clientSecret,
                grant_type: 'refresh_token',
            }),
        });

        const data = await response.json();
        console.log('Refresh Result Response:', JSON.stringify(data, null, 2));
    } catch (err) {
        console.error('Error during Zoho refresh exchange:', err);
    }
}

testRefresh();
