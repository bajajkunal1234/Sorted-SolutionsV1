...
export default function SitemapViewer({ staticLinks, appliances, locations }) {
    const toLabel = (slug) => slug.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase());

    const totalUrls = staticLinks.length
        + appliances.length
            ...
            <div style={{ maxWidth: 1100, margin: '0 auto', padding: '36px 24px 60px' }}>
                ...
                <div className="smap-section">
                    <div className="smap-section-header">
                        ...
                        <span style={{ marginLeft: 'auto', background: '#10b98122', color: '#10b981', borderRadius: 20, padding: '2px 10px', fontSize: 12, fontWeight: 700 }}>{appliances.length} pages</span>
                    </div>
                    <div className="smap-grid" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))' }}>
                        {appliances.map(a => <Card key={a.slug} label={`${a.name || toLabel(a.slug)} Repair`} url={`/services/${a.slug}`} color="green" />)}
                    </div>
                </div>
                ...
                <div className="smap-section">
                    <div className="smap-section-header">
                        ...
                        <span style={{ marginLeft: 'auto', background: '#f59e0b22', color: '#f59e0b', borderRadius: 20, padding: '2px 10px', fontSize: 12, fontWeight: 700 }}>{appliances.reduce((a, c) => a + c.subcategories.length, 0)} pages</span>
                    </div>
                    {appliances.map(a => (
                        <div key={a.slug} style={{ marginBottom: 20 }}>
                            <div className="smap-sub-label" style={{ color: '#f59e0b' }}>{a.name || toLabel(a.slug)}</div>
                            <div className="smap-grid" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))' }}>
                                {a.subcategories.map(s => <Card key={s.slug} label={s.name || toLabel(s.slug)} url={`/services/${a.slug}/${s.slug}`} color="amber" sm />)}
                            </div>
                        </div>
                    ))}
                </div>
                ...
                <div className="smap-section">
                    ...
                    <div className="smap-grid" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))' }}>
                        {locations.map(loc => <Card key={loc} label={toLabel(loc)} url={`/location/${loc}`} color="pink" sm />)}
                    </div>
                </div>
                ...
                <div className="smap-section">
                    <div className="smap-section-header">
                        ...
                        <span style={{ marginLeft: 'auto', background: '#0ea5e922', color: '#0ea5e9', borderRadius: 20, padding: '2px 10px', fontSize: 12, fontWeight: 700 }}>{locations.length * appliances.length} pages</span>
                    </div>
                    <p style={{ color: '#64748b', fontSize: 13, margin: '0 0 16px' }}>{appliances.length} services × {locations.length} locations</p>
                    {appliances.map(a => (
                        <div key={a.slug} style={{ marginBottom: 20 }}>
                            <div className="smap-sub-label" style={{ color: '#0ea5e9' }}>{a.name || toLabel(a.slug)}</div>
                            <div className="smap-grid" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))' }}>
                                {locations.map(loc => <Card key={loc} label={`${a.name || toLabel(a.slug)} in ${toLabel(loc)}`} url={`/location/${loc}/${a.slug}`} color="sky" sm />)}
                            </div>
                        </div>
                    ))}
                </div>
