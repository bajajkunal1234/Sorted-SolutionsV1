...
import { Search, Plus, Trash2, Edit2, Save, X, Globe, Tag, FileText, Code, Loader2, RefreshCcw } from 'lucide-react';
...
function SEOSettings() {
    ...
    const fetchSettings = async () => {
        try {
            setLoading(true);
            const [metaData, schemaData, keywordsData] = await Promise.all([
                websiteSettingsAPI.getByKey('seo-meta-tags'),
                websiteSettingsAPI.getByKey('seo-schema-markup'),
                websiteSettingsAPI.getByKey('seo-keywords')
            ]);

            if (metaData && metaData.value) setMetaTags(metaData.value);
            if (schemaData && schemaData.value) setSchemaMarkup(schemaData.value);
            if (keywordsData && keywordsData.value) setSeoKeywords(keywordsData.value);
        } catch (err) {
    ...
    const handleSaveAll = async () => {
        try {
            setSaving(true);
            await Promise.all([
                websiteSettingsAPI.save('seo-meta-tags', metaTags, 'Global meta tags for SEO'),
                websiteSettingsAPI.save('seo-schema-markup', schemaMarkup, 'Structured data schema markup'),
                websiteSettingsAPI.save('seo-keywords', seoKeywords, 'Target SEO keywords library')
            ]);
        ...
        <div>
            ...
            {loading ? (
                <div style={{ padding: 'var(--spacing-2xl)', textAlign: 'center' }}>
                    ...
                    <p style={{ color: 'var(--text-secondary)' }}>Loading SEO settings...</p>
                </div>
            ) : (
                <>
