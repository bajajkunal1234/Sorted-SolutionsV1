...
import { Package, Tag, BarChart2, TrendingUp, TrendingDown, Filter, RefreshCcw, Search } from 'lucide-react';
...
function InventoryReports() {
    ...
    const totalStockValue = filtered
        .filter(p => p.type === 'product')
        .reduce((sum, p) => sum + ((p.current_stock || 0) * (p.purchase_price || 0)), 0);
    const totalSaleValue = filtered
        .filter(p => p.type === 'product')
        .reduce((sum, p) => sum + ((p.current_stock || 0) * (p.sale_price || 0)), 0);
    ...
    // Also catch items with uncategorized/string categories not in categories list
                        ...
                        {activeReport === 'overview' && (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-lg)' }}>
                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 'var(--spacing-md)' }}>
                                    ...
                                    <StatCard label="Stock Cost Value" value={fmt(totalStockValue)} color="#f59e0b" sub="At purchase price" />
                                    ...
                                    <div style={{ overflowX: 'auto' }}>
                                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 'var(--font-size-xs)' }}>
                                            <thead style={{ backgroundColor: 'var(--bg-secondary)' }}>
                                                <tr>
                                                    {['SKU', 'Name', 'Type', 'Category', 'Brand', 'Stock', 'Sale Price', 'Purchase Price', 'Stock Value', 'Status'].map(h => (
                                                        <th key={h} style={{ padding: '8px 12px', textAlign: h === 'Stock' || h.includes('Price') || h.includes('Value') ? 'right' : 'left', fontWeight: 600, whiteSpace: 'nowrap' }}>{h}</th>
                                                    ))}
                                                </tr>
                                            </thead>
                                            <tbody>
                                    ...
                                    <div key={cat.name} style={{ backgroundColor: 'var(--bg-elevated)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-primary)', overflow: 'hidden' }}>
                                        <div style={{ padding: 'var(--spacing-sm) var(--spacing-md)', borderBottom: '1px solid var(--border-primary)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: 'var(--bg-secondary)' }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-sm)' }}>
                                                ...
                                                <span style={{ fontSize: 'var(--font-size-xs)', color: 'var(--text-tertiary)', backgroundColor: 'var(--bg-tertiary)', padding: '1px 6px', borderRadius: '10px' }}>
                                                    {cat.count} item{cat.count !== 1 ? 's' : ''}
                                                </span>
                                            </div>
                                            <span style={{ fontSize: 'var(--font-size-sm)', fontWeight: 700, color: '#10b981' }}>
                                                {fmt(cat.stockValue)} stock value
                                            </span>
                                        </div>
                                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 'var(--font-size-xs)' }}>
                                            <thead style={{ backgroundColor: 'var(--bg-tertiary)' }}>
                                                <tr>
                                                    {['SKU', 'Name', 'Brand', 'Stock', 'Sale Price', 'Purchase Price', 'Value'].map(h => (
                                                        <th key={h} style={{ padding: '6px 12px', textAlign: h === 'Stock' || h.includes('Price') || h === 'Value' ? 'right' : 'left', fontWeight: 600 }}>{h}</th>
                                                    ))}
                                                </tr>
                                            </thead>
                                            <tbody>
                                ...
                                {byBrand.length === 0 ? (
                                    ...
                                    <div key={b.name} style={{ backgroundColor: 'var(--bg-elevated)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-primary)', overflow: 'hidden' }}>
                                        ...
                                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 'var(--font-size-xs)' }}>
                                            <thead style={{ backgroundColor: 'var(--bg-tertiary)' }}>
                                                <tr>
                                                    {['SKU', 'Name', 'Category', 'Stock', 'Sale Price', 'Purchase Price', 'Cost Value', 'Sale Value'].map(h => (
                                                        <th key={h} style={{ padding: '6px 12px', textAlign: h === 'Stock' || h.includes('Price') || h.includes('Value') ? 'right' : 'left', fontWeight: 600 }}>{h}</th>
                                                    ))}
                                                </tr>
                                            </thead>
                                            <tbody>
                                ...
                                {[{ label: 'Out of Stock', items: outOfStock, color: '#ef4444' }, { label: 'Low Stock', items: lowStock, color: '#f59e0b' }, { label: 'In Stock', items: inStock, color: '#10b981' }].map(group => group.items.length > 0 && (
                                    <div key={group.label} style={{ backgroundColor: 'var(--bg-elevated)', borderRadius: 'var(--radius-md)', border: `1px solid ${group.color}30`, overflow: 'hidden' }}>
                                        ...
                                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 'var(--font-size-xs)' }}>
                                            <thead style={{ backgroundColor: 'var(--bg-tertiary)' }}>
                                                <tr>
                                                    {['SKU', 'Name', 'Category', 'Brand', 'Current Stock', 'Reorder Level', 'Sale Price'].map(h => (
                                                        <th key={h} style={{ padding: '6px 12px', textAlign: h.includes('Stock') || h.includes('Level') || h.includes('Price') ? 'right' : 'left', fontWeight: 600 }}>{h}</th>
                                                    ))}
                                                </tr>
                                            </thead>
                                            <tbody>
                        ...
                        {activeReport === 'valuation' && (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-lg)' }}>
                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 'var(--spacing-md)' }}>
                                    <StatCard label="Total Cost Value" value={fmt(totalStockValue)} color="#6366f1" sub="Stock × Purchase Price" />
                                ...
                                <div style={{ backgroundColor: 'var(--bg-elevated)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-primary)', overflow: 'hidden' }}>
                                    ...
                                    <div style={{ overflowX: 'auto' }}>
                                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 'var(--font-size-xs)' }}>
                                            <thead style={{ backgroundColor: 'var(--bg-secondary)' }}>
                                                <tr>
                                                    {['SKU', 'Name', 'Category', 'Brand', 'Stock', 'Purchase Price', 'Sale Price', 'Cost Value', 'Retail Value', 'Margin'].map(h => (
                                                        <th key={h} style={{ padding: '8px 12px', textAlign: h === 'Stock' || h.includes('Price') || h.includes('Value') || h === 'Margin' ? 'right' : 'left', fontWeight: 600, whiteSpace: 'nowrap' }}>{h}</th>
                                                    ))}
                                                </tr>
                                            </thead>
                                            <tbody>
